...
logDebug("\n\n" . str_repeat("=", 150));
logDebug("🚀 PROXY.PHP INITIALIZED - " . date('Y-m-d H:i:s'));
...
try {
    ...
    if ($isTokenRequest || empty($data)) {
        ...
        if (empty($authUrl) || empty($executionUrl)) {
            throw new Exception("API URLs not configured in database or .env file");
        }
        ...
        if (empty($executionUrl)) {
            throw new Exception("Action API URL not configured in database or .env file");
        }
